...
function ValueIsNumberInRange(Value, minValue, maxValue, withMin, withMax) {
    ...
    if (ValueIsFiniteNumber(minValue)) {
        // more robust than "isFinite" alone
        if (ValueIsFiniteNumber(maxValue)) {
            // more robust than "isFinite" alone
            if (
                Value < minValue ||
                (!withMin && Value === minValue) ||
                Value > maxValue ||
                (!withMax && Value === maxValue)
            ) {
                return false;
            }
        } else {
...
function ValueIsColor(Value) {
    return (
        ...
        (ColorSet.hasOwnProperty(Value) ||
            ...
            /^rgba\([0-9]+,\s*[0-9]+,\s*[0-9]+,([01]|[0]?[.][0-9]+)\)$/.test(
                Value,
            )) // dto.
    );
}
...
function validatedArgument(
    ...
    if (Argument == null) {
        if (NilIsAcceptable) {
            ...
            throwError(
                "MissingArgument: no ".concat(escaped(Description), " given"),
            );
        }
    } else {
        if (ValueIsValid(Argument)) {
            ...
            throwError(
                "InvalidArgument: the given "
                    .concat(escaped(Description), " is no valid ")
                    .concat(escaped(Expectation)),
            );
        }
    }
}
...
function FunctionWithName(originalFunction, desiredName) {
    ...
    if (typeof originalFunction !== "function") {
        throwError(
            "InvalidArgument: the given 1st Argument is not a JavaScript function",
        );
    }
    if (desiredName == null) {
    ...
    try {
        ...
        /* ok - let's take the hard way */
    }
    var renamed = new Function(
        "originalFunction",
        "return function " +
            desiredName +
            " () {" +
...
var allowFiniteNumber = /*#__PURE__*/ ValidatorForClassifier(
    ValueIsFiniteNumber,
    acceptNil,
    "finite numeric value",
);
var expectFiniteNumber = /*#__PURE__*/ ValidatorForClassifier(
    ValueIsFiniteNumber,
    rejectNil,
    "finite numeric value",
);
...
function expectOneOf(Description, Argument, ValueList) {
    ...
    if (ValueIsOneOf(Argument, ValueList)) {
        ...
        throwError(
            "InvalidArgument: the given ".concat(
                escaped(Description),
...
function escaped(Text) {
    var EscapeSequencePattern =
        /\\x[0-9a-zA-Z]{2}|\\u[0-9a-zA-Z]{4}|\\[0bfnrtv'"\\\/]?/g;
...
var allowPosition = ValidatorForClassifier(
        ValueIsPosition,
        acceptNil,
        "turtle position",
    ),
    allowedPosition = allowPosition;
var expectPosition = ValidatorForClassifier(
        ValueIsPosition,
        rejectNil,
        "turtle position",
    ),
...
var allowAlignment = ValidatorForClassifier(
        ValueIsAlignment,
        acceptNil,
        "turtle alignment",
    ),
    allowedAlignment = allowAlignment;
var expectAlignment = ValidatorForClassifier(
        ValueIsAlignment,
        rejectNil,
        "turtle alignment",
    ),
...
function ValueIsPathOptionSet(Value) {
    return (
        ...
        (Value.Lineature == null ||
            ValueIsOneOf(Value.Lineature, TUR_Lineatures)) &&
...
var allowPathOptionSet = ValidatorForClassifier(
        ValueIsPathOptionSet,
        acceptNil,
        "turtle path option set",
    ),
    allowedPathOptionSet = allowPathOptionSet;
var expectPathOptionSet = ValidatorForClassifier(
        ValueIsPathOptionSet,
        rejectNil,
        "turtle path option set",
    ),
...
var Graphic = /** @class */ (function () {
    ...
    Graphic.prototype._curve = function (Angle, rx, ry, clockwise) {
        ...
        var x1 =
            cx +
            auxX * cos(DirectionInRadians) -
            auxY * sin(DirectionInRadians);
        var y1 =
            cy +
            auxX * sin(DirectionInRadians) +
            auxY * cos(DirectionInRadians);
        /**** construct SVG path ****/
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (fullEllipse) {
                ...
                var maxX =
                    x * cos(-DirectionInRadians) - y * sin(-DirectionInRadians);
                var maxY =
                    x * sin(-DirectionInRadians) + y * cos(-DirectionInRadians);
                maxX = maxX / rx;
                maxY = maxY / ry;
        ...
        this.currentDirection +=
            (Angle >= 0 ? Angle : 180 + Angle) * (clockwise ? 1 : -1);
        this.currentX = x1;
        this.currentY = y1;
        this._updateBoundingBox(
    ...
    Graphic.prototype.asSVGwith72dpi = function (Unit, xMin, yMin, xMax, yMax) {
        ...
        return (
            ...
            ceiled(Scale * (xMax - xMin)) +
            " " +
            ceiled(Scale * (yMax - yMin)) +
            '" ' +
            'vector-effect="non-scaling-stroke"' +
            ">" +
            '<g transform="scale(' +
...
export {
    ...
    allowPosition,
    allowedAlignment,
    allowedPathOptionSet,
    allowedPosition,
    expectAlignment,
    expectPathOptionSet,
    expectPosition,
    expectedAlignment,
    expectedPathOptionSet,
    expectedPosition,
};
